...
class verificator():
    ...
    # 多条产权数据校验（内蒙古允许）
    def tooManyCqResultVerify(self,cqType,queryData,*args):
        '''
    ...
    # 产权是否已登记校验
    def cqIsRegisterVerify(self,cqType,queryData,*args):
        '''
        :param cqType: 0 净地；1 房地
        :param queryData: 查询入参
        :param *args: tuple  0 内蒙；1 江苏
        :return:
        '''
        if cqType == 0:
            ...
            queryFdcqSQL = "select count(1) from dj_fdcq2 where zt='1' and sfyx=1 and bdcdyh='" + queryData + "'"
            queryFdcqSQLRes = self.djObj.fetchone(queryFdcqSQL)
            if queryFdcqSQLRes:
                logger.warning("登记平台该房屋信息已登记")
                return
            return True
