...
class dataResCheck():
    ...
    def landRegisterDataCheck(self,bdcdyh,data):
        ...
        try:
            ...
            sql_dj_qlr = "select count(1) from dj_qlr where zt='1' and sfyx=1 and id =(select ryqkid from dj_qlrgl where zt='1' and sfyx=1 and qlbid='"+ res_dj_jsydsyq_id + "')"
    ...
    def landCancelRegisterDataCheck(self, bdcdyh, data):
        ...
        try:
            ...
            sql_dj_djben = "select count(1) from dj_djben where zt='1' and sfyx=1 and sfzx=1  and id =" + res_dj_jsydsyq_djbid + ""
            res_dj_djben = self.db_dj_conn.SqlExecute(sql_dj_djben)
            logger.debug("dj_djben表查询sql：%s" % sql_dj_djben)
            logger.debug("dj_djben表查询记录数：%d" % res_dj_djben)
    ...
    def spfYdyRegisterDataCheck(self, bdcdyh, data):
        ...
        try:
            ...
            sql_dj_djben = "select count(1) from dj_djben where zt='1' and sfyx=1 and sfysczql=0 and id =" + res_dj_ydy_djbid + ""
            res_dj_djben = self.db_dj_conn.SqlExecute(sql_dj_djben)
            logger.debug("dj_djben表查询sql：%s" % sql_dj_djben)
            logger.debug("dj_djben表查询记录数：%d" % res_dj_djben)
