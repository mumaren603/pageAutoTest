...
@pytest.mark.test
@pytest.mark.all
class Test_housePlxcfRegister():
    def setup(self):
        '''初始化用户数据获取'''
        current_file_path = os.path.abspath(__file__).replace('\\', '/')
        self.data = getTestcaseData(getTestdataPath(current_file_path))

    def test_housePlxcfRegister(self,login,cmdopt):
        self.driver = login[0]
        dbInfo = login[1]
        # 获取办件数据
        bdcdyh = dataInit().getHouseXcfRegisterData()
        logger.debug("<--------查封登记--查封--批量续查封登记（房屋）start-------->")
        ...
        try:
            ...
            raise
        logger.debug("<--------查封登记--查封--批量续查封登记（房屋）end-------->")
...
if __name__ == '__main__':
    pytest.main(['-v', 'test_housePlxcfRegister'])
