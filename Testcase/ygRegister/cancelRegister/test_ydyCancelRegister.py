...
@pytest.mark.test
@pytest.mark.all
class Test_ydyCancelRegister():
    def setup(self):
        '''初始化用户数据获取'''
        current_file_path = os.path.abspath(__file__).replace('\\','/')
        self.data = getTestcaseData(getTestdataPath(current_file_path))

    def test_ydyCancelRegister(self,login,cmdopt):
        '''
        :流程 预告登记--注销登记--预告抵押注销登记
        '''
        self.driver = login[0]
        dbInfo = login[1]
        # 获取办件数据
        bdcdyh = dataInit().getYdyCancelRegisterData()
        ...
        try:
            ...
            resDataCheck = dataResCheck().ydyCancelRegisterDataCheck(bdcdyh, self.data)
            assert resDataCheck
...
if __name__ == '__main__':
    pytest.main(['-v', 'test_ydyCancelRegister'])
