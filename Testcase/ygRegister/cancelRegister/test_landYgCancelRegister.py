...
class Test_landYgCancelRegister():
    ...
    def test_landYgCancelRegister(self,login,cmdopt):
        ...
        dbInfo = login[1]
        # 获取办件数据
        bdcdyh = dataInit().getYgCancelRegisterData(self.data)
        logger.debug("<--------预告登记--注销登记--净地预告注销登记start-------->")
        ...
        try:
            ...
            raise
        logger.debug("<--------预告登记--注销登记--净地预告注销登记end-------->")
...
if __name__ == '__main__':
    pytest.main(['-v', 'test_landYgCancelRegister'])
