...
class Test_landDyCancelRegister():
    ...
    def test_landDyCancelRegister(self,login,cmdopt):
        ...
        dbInfo = login[1]
        # 获取办件数据
        bdcdyh = dataInit().getLandDyRegisterData()
        logger.debug("<--------抵押权--注销登记--净地抵押注销start-------->")
        ...
        try:
            ...
            resDataCheck = dataResCheck().dyCancelRegisterDataCheck(bdcdyh, self.data)
            assert resDataCheck
            logger.debug("<--------归档数据检查end-------->")
        except AssertionError:
            raise
        logger.debug("<--------抵押权--注销登记--净地抵押注销end-------->")
...
if __name__ == '__main__':
    pytest.main(['-v', 'test_landDyCancelRegister'])
