...
class dataInit():
    ...
    def getHouseYcfRegisterData(self):
        logger.debug("<--------查询入参数据start-------->")
        querySQL="select a.bdcdyh from " \
                 "(select bdcdyh, djbid from dj_yg where zt = '1' and sfyx = 1  and bdcdyh not like '%9999%') a " \
                 "inner join " \
                 "(select id from dj_djben where sfyg = 1 and sfycf = 0 and sfysczql = 0 and zt = '1' and sfyx = 1) b " \
                 "on a.djbid = b.id " \
                 "inner join " \
                 "(select bdcdyh from dj_ychxx where zt = '1' and sfyx = 1) c " \
    ...
    def getHouseSfcdRegisterData(self):
        ...
        if queryRes:
            ...
            if queryFdcq2SQLRes > 1 or queryHxxSQLRes >1:
                logger.warning("该单元存在多条现势数据，请检查！")
                return dataInit(self.dbInfo).getHouseSfcdRegisterData()
            # 检查该数据是否存在待办件
            querySqxxSQL = "select count(1) from yw_sqxx where ajzt='1' and sfyx=1 and bdcdyh='" + queryRes + "'"
            querySqxxzbSQL = "select count(1) from yw_sqxx t where t.SFYX = '1' and t.ajzt in ('1', '3', '6') and t.id in (select z.sqbid from yw_sqxxzb z where z.sfyx = '1' and bdcdyh = '" + queryRes + "')"
            querySqxxSQLRes = self.db_dj_conn.SqlExecute(querySqxxSQL)
